...
<body>
    ...
    <x-layout>
        <div class="bg-[#f7e8d1] m-auto my-10 w-[80%] max-w-[800px] p-10 rounded-3xl  grid lg:grid-cols-2 gap-10">
            ...
            <form action="{{route('medication.edit',$medication ->id)}}" method="POST" enctype="multipart/form-data" data-aos="zoom-in">
                @csrf
                @method('put')
                <label for="name" class="text-[#888b95] text-sm m-2 ">
                    Name Of Drug
                </label>
                <input type="text" class="input" name="name" id='name' value="{{ $medication->name }}"  placeholder="Enter the name of drug">
                <label for="name" class="text-[#888b95] text-sm m-2 ">
                    Type
                </label>
                <input type="Type" class="input" name="type" id='type' value="{{ $medication->type }}"  placeholder="e.g AntiBiotics, anti-viral e.t.c">
                <label for="name" class="text-[#888b95] text-sm m-2 ">
                    Active Ingridients
                </label>
                <textarea class="input h-40 p-5" name="active" id='active'  >{{ $medication->active }}</textarea>
                <label for="name" class="text-[#888b95] text-sm m-2 ">
                    Approved use of Medication
                </label>
                <textarea class="input h-40 p-5" name="indication" id='indication'  placeholder="e.g Fever, malaria etc">{{$medication->indication}}</textarea>
                <label for="name" class="text-[#888b95] text-sm m-2 ">
                    Manufacturer
                </label>
                <input type="text" class="input" name="manufacturer" id='manufacturer' value="{{ $medication ->manufacturer }}" required placeholder="Enter Address">
                <label for="side" class="text-[#888b95] text-sm m-2 ">
                    Side Effects
                </label>
                <textarea class="input h-40 p-5 rounded-lg" name="side" id='side'  placeholder="e.g  Nausea, Dizziness etc">{{$medication->side}}</textarea>
